...
import { contextPromptDiagnostico, contextSchemaDiagnostico } from "@/lib/context";
...
/**
 * Le o arquivo 1 (diagnostico) e devolve so essa fatia do briefing. Ver
 * lib/context.ts para o porque de duas chamadas separadas em vez de uma.
 */
export async function POST(req: Request) {
  ...
  try {
    const prompts = contextPromptDiagnostico(extraido.texto);
    const bruto = await generateJson(prompts.system, prompts.user, 2500, "conteudo", 58_000);
    const parsed = contextSchemaDiagnostico.safeParse(bruto);
    if (!parsed.success) {
